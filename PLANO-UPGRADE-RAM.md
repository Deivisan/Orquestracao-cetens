# 🚀 PLANO DE UPGRADE RAM - CETENS UFRB

**Data:** 24 de novembro de 2025  
**Responsável:** Deivison Santana (DevSan)  
**Analista:** Tiago Athayde  
**Objetivo:** **Maximizar RAM com recursos atuais** (8GB onde possível, senão 6GB/7GB nos slots vazios)  
**Status Atual:** 61/147 PCs com 8GB (41%) | 86 PCs pendentes (59%)

---

## 📊 INVENTÁRIO DE MEMÓRIAS DISPONÍVEIS

### ✅ DDR3 Compatíveis (95 unidades restantes)
| **Capacidade** | **Quantidade** | **Tipo** | **Compatibilidade** |
|----------------|----------------|----------|---------------------|
| 4GB SO-DIMM | 5 | Notebook | Dell Latitude E5440 |
| 2GB DIMM | 18 | PC | Dell OptiPlex 7010 (máx. 4 slots) |
| 1GB DIMM | 72 | PC | Dell OptiPlex 7010 (máx. 4 slots) |
| 512MB DIMM | 1 | PC | Reserva emergencial |

**💡 Estratégia Inteligente:** Melhor cenário com 2GB+1GB: 36 PCs a 6GB (72x1GB) + 9 PCs a 8GB (18x2GB) = **45 PCs upgraded**. Focar em 6GB para mais PCs, usar 2GB para 8GB prioritários.

---

## ✅ SETORES JÁ COM 8GB RAM (EXCLUÍDOS DO PLANO)

### 🎯 Setores Atualizados (24/11/2025)
- **LAB 01 Informática 1:** 20 PCs Dell OptiPlex 7010 (já 8GB)
- **LAB 07 Informática 2:** 20 PCs Ryzen 7 5700G (já 8GB)
- **LAB 04 Física:** 6 PCs Dell OptiPlex 7010 (upgraded 24/11)
- **LAB 06 Física 2:** 7 PCs Dell OptiPlex 7010 (upgraded 24/11)
- **LAB 101 Maker:** 2 PCs (upgraded 24/11)
- **LAB 105 LIMM:** 6 PCs (upgraded 24/11)

**Total:** 61 PCs já com 8GB - **Não precisam de upgrade RAM**

---

## 🎯 PRIORIDADES DE UPGRADE (AJUSTADO PARA MÁXIMO POSSÍVEL)

### 🔥 **PRIORIDADE 1 - CRÍTICA** (Labs de Pesquisa/Aulas Intensas)

#### 🧪 LAB 03 - NETA-A (Tecnologia Assistiva)
- **PCs:** 3 Dell Optiplex 7010 (4GB → **8GB possível**)
- **Opções:** [ ] 2x2GB por PC (4 módulos) → 8GB | [ ] 4x1GB por PC (12 módulos) → 4GB
- **Prioridade:** 🟡 Média | **Impacto:** Acessibilidade

#### 📐 LAB 202 - Ensino Matemática e Ciências
- **PCs:** 2 (4GB → **8GB possível**)
- **Opções:** [ ] 2x2GB por PC (4 módulos) → 8GB | [ ] 4x1GB por PC (8 módulos) → 4GB
- **Prioridade:** 🟢 Baixa | **Impacto:** Aulas teóricas

#### ⚡ LAB 204 - Elétrica e Eletrônica
- **PCs:** 1 (Verificar RAM → **8GB possível**)
- **Opções:** [ ] Verificar atual + 2x2GB (4 módulos) → 8GB
- **Prioridade:** 🟡 Média | **Impacto:** Experimentos

### 🟡 **PRIORIDADE 2 - ALTA** (Administrativo/Escritórios)

#### 👥 Pavilhão de Gabinetes (25 PCs Dell Optiplex 7010)
- **PCs:** 22 (4GB atual → **6GB/7GB possível**)
- **Config Atual:** 2x2GB (4GB) + 2 slots vazios
- **Opções:** [ ] 2x1GB nos vazios (2 módulos) → 6GB | [ ] 1x2GB + 1x1GB nos vazios (2 módulos) → 7GB | [ ] 4x1GB total (substituir) → 4GB
- **Prioridade:** 🟢 Baixa | **Impacto:** Produtividade docente

#### 📖 Biblioteca (12 PCs Dell Optiplex 7010)
- **PCs:** 12 (4GB atual → **6GB/7GB possível**)
- **Config Atual:** 2x2GB (4GB) + 2 slots vazios
- **Opções:** [ ] 2x1GB nos vazios (2 módulos) → 6GB | [ ] 1x2GB + 1x1GB nos vazios (2 módulos) → 7GB | [ ] 4x1GB total (substituir) → 4GB
- **Prioridade:** 🟢 Baixa | **Impacto:** Pesquisa

#### 🏢 Prédio Administrativo (4 PCs)
- **Outros:** Empresa Júnior, SIAE, etc. (4 PCs)
- **Config Atual:** 2x2GB (4GB) + 2 slots vazios
- **Opções:** [ ] 2x1GB nos vazios (2 módulos) → 6GB | [ ] 1x2GB + 1x1GB nos vazios (2 módulos) → 7GB | [ ] 4x1GB total (substituir) → 4GB
- **Prioridade:** 🔴 Muito Alta | **Impacto:** Dual-boot

### 🟢 **PRIORIDADE 3 - MÉDIA** (Setores Secundários)

#### 🏫 Pavilhão Labs 1 (1 PC)
- **NUGTESP:** 1 PC (4GB atual → **6GB/7GB possível**)
- **Config Atual:** 2x2GB (4GB) + 2 slots vazios
- **Opções:** [ ] 2x1GB nos vazios (2 módulos) → 6GB | [ ] 1x2GB + 1x1GB nos vazios (2 módulos) → 7GB
- **Prioridade:** 🟡 Média

#### 🔬 Pavilhão Labs 2 (11 PCs)
- **LAB 103:** 2 PCs | **LAB Agroecologia:** 2 PCs | **LAB 104:** 7 PCs
- **Config Atual:** 2x2GB (4GB) + 2 slots vazios
- **Opções:** [ ] 2x1GB nos vazios (2 módulos) → 6GB | [ ] 1x2GB + 1x1GB nos vazios (2 módulos) → 7GB | [ ] 4x1GB total (substituir) → 4GB
- **Prioridade:** 🟡 Média

#### 🏢 Prédio Multiuso (7 PCs)
- **Coordenações 1-7:** 7 PCs (4GB atual → **6GB/7GB possível**)
- **Config Atual:** 2x2GB (4GB) + 2 slots vazios
- **Opções:** [ ] 2x1GB nos vazios (2 módulos) → 6GB | [ ] 1x2GB + 1x1GB nos vazios (2 módulos) → 7GB | [ ] 4x1GB total (substituir) → 4GB
- **Prioridade:** 🟢 Baixa

#### 🚪 Outros Setores (4 PCs)
- **Setor Acadêmico, Portaria, Auditório, Pessoal Terceirizado**
- **Config Atual:** 2x2GB (4GB) + 2 slots vazios
- **Opções:** [ ] 2x1GB nos vazios (2 módulos) → 6GB | [ ] 1x2GB + 1x1GB nos vazios (2 módulos) → 7GB | [ ] 4x1GB total (substituir) → 4GB
- **Prioridade:** 🟢 Baixa

---

## 📈 ESTRATÉGIAS DE IMPLEMENTAÇÃO (REALISTA)

### 🎯 Abordagem Recomendada
1. **Fase 1:** Prioridade 1 - 4 PCs a 8GB (usar 2GB) + 2 PCs a 6GB (usar 1GB)
2. **Fase 2:** Prioridade 2 - 9 PCs a 8GB (usar restantes 2GB) + 29 PCs a 6GB (usar 1GB)
3. **Fase 3:** Prioridade 3 - 12 PCs a 6GB (usar restantes 1GB)

### ⚠️ Considerações Técnicas
- **Slots:** Máximo 4 por PC (OptiPlex 7010) - PCs atuais: 2 ocupados (2x2GB) + 2 vazios
- **Compatibilidade:** DDR3-1600 (1280MHz)
- **Testes:** MemTest86 pós-instalação
- **Limitação:** Com 18x2GB, apenas 4 PCs a 8GB; para 7GB, usar 2GB + 1GB por PC nos vazios

### 📋 Checklist Geral
- [ ] **Inventário:** Verificar RAM atual em pendentes (confirmar 2x2GB + 2 vazios)
- [ ] **Priorização:** Confirmar ordem com Tiago Athayde (focar 6GB para mais PCs)
- [ ] **Logística:** Planejar intervenções (evitar aulas)
- [ ] **Documentação:** Atualizar após upgrades
- [ ] **Próximos Passos:** Executar fases sequenciais com recursos atuais

---

**Meta Realista:** Melhor cenário: **45 PCs upgraded** (36x6GB + 9x8GB) | Restantes 16 PCs ficam com 4GB  
**Recursos Atuais:** 72x1GB + 18x2GB = Suficiente para 45 PCs!  

*Documento dinâmico - Atualizado conforme execuções.*</content>
<parameter name="filePath">c:\Projetos\Orquestracao-cetens\PLANO-UPGRADE-RAM.md